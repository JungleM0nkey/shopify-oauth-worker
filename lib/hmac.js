/**
 * HMAC verification utilities for Shopify webhook and OAuth security
 * @module hmac
 */
...
/**
 * Verifies HMAC signature for Shopify OAuth callbacks
 * @param {URLSearchParams} params - Query parameters from OAuth callback
 * @param {string} secret - Shopify app secret for HMAC verification
 * @returns {Promise<boolean>} True if HMAC signature is valid
 */
 ...
 * @param {string} body - Raw webhook body content
 * @param {string} hmac - HMAC signature from X-Shopify-Hmac-Sha256 header
 * @param {string} secret - Shopify app secret for HMAC verification
 * @returns {Promise<boolean>} True if webhook HMAC signature is valid
 */
