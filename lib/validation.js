...
import { getShopData } from './shopify.js';
...
// Check Installation Status with encryption support
export async function checkInstallation(shop, env, logger = null) {
  try {
    const shopData = await getShopData(shop, env, logger);
    return !!shopData;
  } catch (error) {
    if (logger) {
      logger.error('Error checking installation', { shop, error: error.message });
    } else {
